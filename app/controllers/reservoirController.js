...
module.exports.rd_data_latest = function(req, res,next) {
          mongoose.model('Ws').find({}).sort({date: -1}).limit(150).exec(function (err, ws) {
              if (err) {
...
module.exports.rd_data = function(req, res,next) {
        mongoose.model('Ws').find( {}).sort({date: -1}).exec(function (err, ws) {
              if (err) {
                  return console.error(err);
              } else {
                  res.format({
...
//app.get('/api/data/1/UCSCistern1/bl2', function(req, res, next) {
        mongoose.model('Ws').find({"sensorid": "UCSCistern10", "blocoid": "10"}).sort({date: -1}).exec(function (err, ws) {
              if (err) {
                  return console.error(err);
              } else {
                  res.format({
...
module.exports.rd_data_R1bl10 = function(req, res,next) {  
        mongoose.model('Ws').find({"sensorid": "UCSReserv10", "blocoid": "10"}).sort({date: -1}).exec(function (err, ws) {
              if (err) {
                  return console.error(err);
              } else {
                  res.format({
